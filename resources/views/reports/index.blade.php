            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <div class="rounded-2xl border border-white/40 bg-white/60 p-6 shadow-lg backdrop-blur-sm">
                    <div class="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-semibold text-gray-900">User Task Report</h3>
                            <p class="text-xs text-gray-500">Per-user tasks by status, priority, overdue, and hours — yours, or any member of a project you manage.</p>
                        </div>
                    </div>

                    <form method="GET" action="{{ route('reports.user-tasks') }}" class="space-y-4">
                        <div>
                            <label for="project" class="block text-xs font-medium text-gray-600">Project (optional — all projects when empty)</label>
                ...
                <div class="rounded-2xl border border-white/40 bg-white/60 p-6 shadow-lg backdrop-blur-sm">
                    <div class="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-semibold text-gray-900">Project Report</h3>
                            <p class="text-xs text-gray-500">Progress, status and priority breakdown, hours, and workload per member.</p>
                        </div>
                    </div>

                    @forelse ($projects as $project)
                        <a href="{{ route('reports.projects.show', $project->id) }}" class="flex items-center justify-between rounded-xl border border-white/40 px-4 py-3 transition hover:bg-white/70">
                            <span class="text-sm font-medium text-gray-800">{{ $project->name }}</span>
                            <span class="text-xs font-semibold text-indigo-600">Open report &rarr;</span>
                        </a>
                    @empty
                        <p class="mt-2 text-sm text-gray-500">You are not a member of any project yet.</p>
                    @endforelse
                </div>
