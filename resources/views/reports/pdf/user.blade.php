...
<body>
    ...
    <div class="meta">
        {{ $user->name }} &middot; {{ $project?->name ?? 'All projects' }} &middot; Generated {{ $generatedAt->toDateTimeString() }}
    </div>
    ...
    <table class="summary">
        ...
        <tr><td class="key">Cancelled tasks</td><td>{{ $cancelled_count }}</td></tr>
        <tr><td class="key">Overdue tasks</td><td>{{ $overdue_count }}</td></tr>
        ...
        <tr><td class="key">Hours logged on completed tasks</td><td>{{ number_format($completed_hours, 1) }}h</td></tr>
    </table>

    <h2>Tasks</h2>
    @if ($tasks->isEmpty())
        <p>No tasks for this report.</p>
    @else
        <table class="data">
            <thead>
                <tr>
