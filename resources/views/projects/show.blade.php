        ...
        <div class="mx-auto grid max-w-7xl gap-6 sm:px-6 lg:grid-cols-3 lg:px-8">
            <section class="bg-white p-6 shadow-sm sm:rounded-lg lg:col-span-2 ">
                ...
                <h3 class="font-semibold text-gray-900 ">Project Name</h3>
                    <span class="rounded-full bg-gray-100 px-3 py-1 text-xs font-medium capitalize text-gray-700  ">{{ $project->status }}</span>
                </div>
                <p class="mt-1 text-lg text-gray-900 ">{{ $project->name }}</p>
                <dl class="mt-6 grid gap-6 border-t border-gray-200 pt-6 text-sm ">
                    <div><dt class="font-medium text-gray-500">Description</dt><dd class="mt-1 whitespace-pre-line text-gray-900 ">{{ $project->description ?: 'No description provided.' }}</dd></div>
                    <div><dt class="font-medium text-gray-500">Status</dt><dd class="mt-1 capitalize text-gray-900 ">{{ $project->status }}</dd></div>
                    <div><dt class="font-medium text-gray-500">Owner</dt><dd class="mt-1 text-gray-900 ">{{ $project->owner->name }}</dd></div>
                    <div><dt class="font-medium text-gray-500">Dates</dt><dd class="mt-1 text-gray-900 ">{{ $project->start_date?->format('M j, Y') ?? 'Not set' }} â€” {{ $project->end_date?->format('M j, Y') ?? 'Not set' }}</dd></div>
                </dl>
            </section>

            <aside class="bg-white p-6 shadow-sm sm:rounded-lg ">
                <h3 class="font-semibold text-gray-900 ">Members ({{ $project->members->count() }})</h3>
                <ul class="mt-4 space-y-4">
                    @forelse ($project->members as $member)
                        <li class="flex items-center justify-between gap-3">
                            <div>
                                <p class="text-sm font-medium text-gray-900 ">{{ $member->name }}</p>
                                <p class="text-xs text-gray-500">Joined {{ $member->pivot->joined_at ? \Illuminate\Support\Carbon::parse($member->pivot->joined_at)->format('M j, Y') : 'â€”' }}</p>
                            </div>
                            <span class="text-xs font-medium text-gray-600 ">{{ $member->pivot->role }}</span>
                        </li>
                    @empty
                    ...
                    <table class="min-w-full divide-y divide-gray-200 ">
                        ...
                        <tbody class="divide-y divide-gray-200 ">
                            @forelse ($project->tasks as $task)
                                <tr><td class="px-3 py-3 text-sm font-medium text-indigo-600"><a href="{{ route('tasks.show', $task) }}">{{ $task->title }}</a></td><td class="px-3 py-3 text-sm text-gray-600 ">{{ $task->status }}</td><td class="px-3 py-3 text-sm text-gray-600 ">{{ $task->assignedUser?->name ?? 'Unassigned' }}</td><td class="px-3 py-3 text-sm text-gray-600 ">{{ $task->due_date?->format('M j, Y') ?? 'â€”' }}</td></tr>
                            @empty
